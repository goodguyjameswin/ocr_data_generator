...
class DataMaker:
    ...
    def __paste(self, bg_img, tl=None, mode='grid', lines=1, intv=None, num_boxes=6):
        ...
        输入:
            ...
            method: 贴图模式
            lines: 贴图行数（grid模式参数）
            intv(tuple or list): 每行贴图间隔系数（以背景图的宽度作为基准，grid模式参数）
            num_boxes: 贴图个数（random模式参数）
        输出:
            result: 生成图片
            rows(list): 标签信息，grid模式下：shape=(真实贴图行数, 真实贴图列数, 4, 2)，此时不一定是张量形式；
            random模式下：shape=(1, 贴图个数, 4, 2)，此时是张量形式
        """
...
if __name__ == '__main__':
    ...
    img_path = "images/background_0004.jpg"
    label_path = "labels/background_0004.txt"
