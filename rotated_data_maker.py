...
def show_image(image, label):
    ...
    # 设置显示窗口
    cv2.namedWindow('image', 0)  # 设置窗口可变
    cv2.moveWindow('image', 1200, 600)  # 移动窗口到屏幕中央，依据分辨率大小修改
    cv2.resizeWindow('image', 460, 460)  # 可视化的窗口大小
...
class DataMaker:
    ...
    def __paste(self, bg_img, tl=None, mode='grid', lines=1, intv=None, num_boxes=6):
        ...
        if mode == 'grid':
            ...
            while True:
                # 若贴图失败次数已达阈值，强制退出
                if times > 100:
                    print("\'random\'模式下，尝试贴图的次数已达到上限，有效贴图的次数仍未达到期望值\n"
                          "请检查图片、贴图区域大小或重新设置贴图起始位置！")
                    break
                new = []
                # 矫正坐标（可调节）
    ...
    def __save_data(self, idx, result, rows):
        ...
        # 图片路径
        image_path = os.path.join(self.sv_img_dir, img_file)
        # 标签路径
        label_path = os.path.join(self.sv_lbl_dir, lbl_file)
        # 存储合成图
